...
use super::config::LjxDataConverterConfig;

pub struct LjxDataConverter {
...
impl LjxDataConverter {
    pub fn create(mut config: LjxDataConverterConfig) -> anyhow::Result<Self> {
        ...
        match &config.ljx_data_path {
            Some(_t) => {}
            None => {
                error!("ljx_data_path が config に設定されていない");
                anyhow::bail!("ljx_data_path が config に設定されていない")
            }
        }
        Ok(Self { config })
    }
    ...
    pub fn execute(self) -> anyhow::Result<()> {
        ...
        info!("保存ディレクトリの作成：{}", self.config.output_dir);

        let info_logger = InformationLogger::new(&self.config)?;
        let mut converter = ConverterLjxToPly::new(&self.config, info_logger)?;

        converter.forward(self.config.y_start_num)?;

        for i in 0..self.config.convert_quantity {
            ...
            converter.backward(self.config.y_overlap)?;
        }
