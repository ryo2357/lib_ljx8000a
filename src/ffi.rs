...
pub fn check_return_code(return_code: LONG) -> anyhow::Result<()> {
    ...
    match return_code_u16 {
        0x0000 => Ok(()),
        0x1000 => anyhow::bail!("Failed to open the communication path"),
        0x1001 => anyhow::bail!("The communication path was not established."),
        0x1002 => anyhow::bail!("Failed to send the command."),
        0x1003 => anyhow::bail!("Failed to receive a response."),
        0x1004 => anyhow::bail!("A timeout occurred while waiting for the response."),
        0x1005 => anyhow::bail!("Failed to allocate memory."),
        0x1006 => anyhow::bail!("An invalid parameter was passed."),
        0x1007 => anyhow::bail!("The received response data was invalid"),
        0x1009 => anyhow::bail!("High-speed communication initialization could not be performed."),
        0x100A => anyhow::bail!("High-speed communication was initialized."),
        0x100B => anyhow::bail!(
            "Error already occurred during high-speed communication (for high-speed communication)"
        ),
        0x100C => anyhow::bail!("The buffer size passed as an argument is insufficient. "),

        0x8081 => anyhow::bail!("送信開始位置と指定されたデータが存在しない"),
        0x80A1 => anyhow::bail!("既に高速データ通信を行っている"),

        0x80A0 => anyhow::bail!("高速データ通信用の接続が確立していない"),
        0x80A2 => anyhow::bail!("高速データ通信開始前準備が行われていない"),
        0x80A3 => anyhow::bail!("高速データ通信開始前準備が行われていない"),
        0x80A4 => anyhow::bail!("送信対象のデータがクリアされた"),
        _ => anyhow::bail!("想定外のリターンコード"),
    }
}
